...
\item \mintinline{scheme}{MkΣ} is the dependent sum data constructor,
  the building block to construct a proof of an existential statement,
...
of the candidates, in this case the class of binary Boolean functions,
and \mintinline{scheme}{(Fit 0.7)} represents the predicate expressing
the class of candidates that are fit with degree of fitness 0.7.  Thus
\mintinline{scheme}{GoodFit} has been replaced by the parameterized
type \mintinline{scheme}{(Fit FITNESS)}.  A proof for such type may
look like
...
where \mintinline{scheme}{\} represents a lambda abstraction.  Note
  ...
  \mintinline{scheme}{\} is not part of the MeTTa syntax.  However,
    JeTTa, a MeTTa compiler for the JVM, seems to already supports it.
    Regardless, in that somewhat made-up and prospective syntax of
    lambda
\begin{minted}{scheme}
(\ $x $y (and $x $y))
\end{minted}
