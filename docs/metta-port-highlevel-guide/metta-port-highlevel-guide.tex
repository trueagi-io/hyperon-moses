...
\item Reducing candidates to normal form.  That trick consists in
  ...
  could be rewritten into \mintinline{scheme}{(and x y)},
  where its arguments have been swapped to follow a predefined order.
  Thus if MOSES
  generates both \mintinline{scheme}{(and x y)} and
  \mintinline{scheme}{(and y x)}, after reduction they would both
...
\item What it outputs is the query itself where the holes have been
  ...
  a candidate.  The best one, appearing last, is
  \mintinline{scheme}{(\ z (s z) (and z (s z)))}, alongside a proof,
  here undefined and represented by \mintinline{scheme}{PRF3}, that
  the candidate has a fitness score of \mintinline{scheme}{0.8}.
...
\mintinline{scheme}{continue-search-deme} make that decision on its
...
code of \mintinline{scheme}{continue-search-deme}, instead of solely
...
\mintinline{scheme}{Prime} appears twice, once as the first argument
of \mintinline{scheme}{MkΣ} and a second time as the second argument
...
sometimes remove the first argument of \mintinline{scheme}{MkΣ} in
...
\end{itemize}
\mintinline{scheme}{PROOF_OF_FITNESS} is left undefined but could be
not that far from an actual definition in a pragmatic use (we will
look into it as we inject computations in that reasoning process to
speed it up).\\
...
\item In order to quote programs, to avoid spontaneous reduction by
  ...
  using the Unicode character set, so that \mintinline{scheme}{and} in
...
\mintinline{scheme}{CPU} to convey this idea.
...
\item \mintinline{scheme}{$enf_prf} is an inhabitant of the sigma
  type, which upon running that query would be replaced by a dependent
  pair with the program in Elegant Normal Form as first argument,
  which would be
  \begin{minted}{scheme}
(\ z (s z) (and z (s z)))
  \end{minted}
  ...
  structured type capturing what it means to be in Elegant Normal
  Form.
\end{itemize}
